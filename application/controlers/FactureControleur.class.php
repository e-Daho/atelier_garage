...
class FactureControleur{
	...
	public function get($idFacture){
		return $this->_factureManager->get($idFacture);
	}
	...
	public function getList(){
		...
		$prixTotal = '%';
		if (!empty($_POST['prixTotal'])) {$prixTotal.=$_POST['prixTotal'].'%';}
		...
		if (!empty($_POST['idFacture']) ) {
			...
			if (!$this->_factureManager->exists($facture)) {
				if($this->_factureManager->add($facture)){
					$out='Le facture numéro '.$_POST['idFacture'].' a bien été ajoutée.';
				}else{
		...
		if (!empty($_POST['idFacture']) ) {
			...
			if ($this->_factureManager->exists($facture)) {
				if($this->_factureManager->update($facture)){
					$out='Le facture numéro '.$_POST['idFacture'].' a bien été modifiée.';
				}else{
					$out='OUPS ! Il y a eu un problème.'; 
				}
			} else {
