...
class TechnicienManager
{
	...
	{
		$q = $this->_db->prepare('INSERT INTO technicien SET numero = :numero, nom = :nom, prenom = :prenom, nombre= :nombre');

		$q->bindValue(':numero',$technicien->numero(),PDO::PARAM_STR);
		$q->bindValue(':nom',$technicien->nom(),PDO::PARAM_STR);
		$q->bindValue(':prenom',$technicien->prenom(),PDO::PARAM_STR);
		$q->bindValue(':nombre',$technicien->nombre(),PDO::PARAM_INT);
		$q->execute();
	...
	{
		$q = $this->_db->prepare('SELECT numero, nom, prenom, nombre FROM technicien WHERE numero = :numero');	
	...
	public function update(Technicien $technicien)
	{
		...
		{
			$q = $this->_db->prepare('UPDATE technicien SET nom = :nom, prenom = :prenom, nombre= :nombre WHERE numero = :numero');
